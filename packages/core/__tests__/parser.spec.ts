...
import { TokenType, tokenize } from '../lib/tokenizer';
...
describe('测试 parser.ts', () => {
  ...
  test('测试 skipRe failed', () => {
    ...
    parser.offset = 100;
    parser.skipRe(new RegExp('message'));
    expect(parser.offset).toBe(100);
  });
...
describe('测试 index.ts', () => {
  ...
  test('测试 parse 无 package', () => {
    const source = readFileSync(join(__dirname, '../../../test/protocol-buffers/comment.proto')).toString();
    const tokens = tokenize(source);
    const root = parse({ source: tokens, filename: 'comment.proto' });

    expect(root.syntax).toBe('proto3');
    expect((root.package as CIdentifier).escapedText).toBe('comment');
  });

  test('测试 parse 长 package', () => {
    const source = readFileSync(join(__dirname, '../../../google/protobuf/any.proto')).toString();
    const tokens = tokenize(source);
    const root = parse({ source: tokens, filename: 'comment.proto' });

    expect(root.syntax).toBe('proto3');
    expect(root.package.kind).toBe(CSyntaxKind.PropertyAccessExpression);

    const pkg = root.package as CPropertyAccessExpression;
    expect(pkg.name.escapedText).toBe('protobuf');
    expect((pkg.expression as CIdentifier).escapedText).toBe('google');
  });
});
