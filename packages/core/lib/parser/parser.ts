...
import { customCreateIdentifier, customCreateNumericLiteral, customCreatePropertyAccessExpression } from '../util';
import {
  CEnumDeclaration,
  CEnumElement,
  CExpression,
...
export default class Parser {
  ...
  parseNumber(token: Token): CNumericLiteral {
    return customCreateNumericLiteral(this.serializeNumber(token).toString())
  }
  ...
  parseImport(): CExpression {
    this.skip('"');
    const token = this.next();
    const syt = new CImportExpression(customCreateIdentifier(token.value));
    this.skip('"');
    return syt;
  }
  ...
  parseService(): CServiceDeclaration {
    ...
    this.skip('{');
    return new CServiceDeclaration(customCreateIdentifier(token.value), []);
  }
  ...
  parseMessage(): CMessageDeclaration {
    ...
    return new CMessageDeclaration(customCreateIdentifier(token.value), []);
  }
  ...
  parseMessageElement(parent: CMessageDeclaration | null): CMessageElement {
    ...
    return new CMessageElement(customCreateIdentifier(name.value), this.parseTypeNode(type.value), parent);
  }
  ...
  parseEnum(): CEnumDeclaration {
    ...
    return new CEnumDeclaration(customCreateIdentifier(token.value), [])
  }
  ...
  parseEnumElement(parent: CEnumDeclaration | null): CEnumElement {
    ...
    this.skip(';');
    return new CEnumElement(customCreateIdentifier(name.value), new CKeyWordTypeNode(CSyntaxKind.NumberKeyWord), this.parseNumber(init), parent);
  }
